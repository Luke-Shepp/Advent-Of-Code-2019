...
class ExpectsInputException extends \Exception
{
    //
}
...
class Computer
{
    ...
    /**
     * Add a string as ASCII values, followed by a new line.
     *
     * @param int $value
     */
    public function addInputString(string $string)
    {
        foreach (str_split($string) as $chr) {
            $this->addInput(ord($chr));
        }
        $this->addInput(10);
    }
     ...
     * @param int $value
     */
    public function writeMemory(int $location, int $value)
    {
        if (!isset($this->opcodes[$location])) {
            $this->increaseMemory($location + 1);
        }
